...
class AuthController extends Controller {
    ...
    public function login(Request $request) {
        ...
        if ($validator->fails()) {
            $key = $validator->errors()->first();
            return responseJson(0, __($validator->errors()->first()));
        }

        try {
            $user = User::with(['plan', 'company'])->where("phone", $request->email)->orWhere("email", $request->email)->first();
        ...
        return responseJson(0, trans("messages_en.phone_or_password_error"));
    }
